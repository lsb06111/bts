...
<body data-ctx="<%=request.getContextPath()%>"
		...
		<div
			...
			<div class="d-flex justify-content-between align-items-center mb-4">
				<div>
					...
					<p style="color: #777; font-size: 0.9rem; margin: 0;">사원 정보와 근무
						상태를 확인하세요.</p>
				</div>
			</div>
			...
			<div
				...
				<div class="d-flex align-items-center gap-2">
					<select class="form-select form-select-sm" name="deptSelect"
						style="width: auto; border-radius: 8px; height: 38px;">
						<option value="">부서</option>
						<option value="개발팀"
							<c:if test="${param.dept == '개발팀'}">selected</c:if>>개발팀
						</option>
						<option value="운영팀"
							<c:if test="${param.dept == '운영팀'}">selected</c:if>>운영팀
						</option>
						<option value="인사팀"
							<c:if test="${param.dept == '인사팀'}">selected</c:if>>인사팀
						</option>
					</select>
				</div>
				...
				<form action="${pageContext.request.contextPath}/emp/list2"
					method="get" class="d-flex align-items-center" style="margin: 0;">

					<!-- 부서 필터 값 동기화용 hidden -->
					<input type="hidden" name="dept" id="deptHidden"
						value="${param.dept}" />

					<div style="position: relative; width: 220px;">
						<input type="text" name="ename" value="${param.ename}"
							class="form-control form-control-sm" placeholder="사원명 검색..."
				...
				<table class="table align-middle text-center mb-0"
					...
					<tbody>
						<c:forEach var="emp" items="${users}">
							<tr class="emp-row" data-empno="${emp.emp.empno}"
								...
								data-position="${emp.emp.position}"
								...
								<td style="color: #555;">${emp.emp.position}(${emp.emp.job.jname})</td>
								<td><c:choose>
										<c:when test="${emp.emp.estate == '재직중'}">
											<span class="badge rounded-pill bg-success"
												style="padding: 6px 10px; font-weight: 500; font-size: 0.8rem;">
												${emp.emp.estate} </span>
										</c:when>

										<c:when test="${emp.emp.estate == '휴가'}">
						...
						<c:if test="${empty users}">
							<tr>
								<td colspan="7" style="padding: 40px 0; color: #aaa;">등록된
									사원 정보가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
			...
			<nav aria-label="Page navigation" class="mt-4">
				<ul class="pagination justify-content-center mb-0">
					...
					<!-- 다음 -->
					<li
						class="page-item ${page >= totalPage || totalPage == 0 ? 'disabled' : ''}">
						<a class="page-link"
						href="?page=${page+1}&ename=${param.ename}&dept=${param.dept}"
	...
	<%@ include
		file="/WEB-INF/views/jspf/employee/employeeUpdateModal2.jspf"%>
	...
	$(document).ready(function() {
	    ...
	    $(document).on("click", ".emp-row", function() {
	    	...
	    	if(!confirm("해당 사원의 정보를 수정하시겠습니까?")){
				return;
			}
	        ...
	        $("#employeeUpdateModal2 #updatePositionInput").val($(this).data("position"));

	        // empno hidden 생성 여부 확인 후 세팅
	        if (!$("#employeeUpdateModal2 #updateEmpnoHidden").length) {
	            $("#employeeUpdateModal2 #empUpdateForm")
	                .append('<input type="hidden" id="updateEmpnoHidden" name="empno">');
	        }
