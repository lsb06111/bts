		...
		<div
			...
			<div class="d-flex justify-content-between align-items-center mb-4">
				<div>
					...
					<p style="color: #777; font-size: 0.9rem; margin: 0;">프로젝트와 저장소
						정보를 관리하세요.</p>
				</div>
			</div>
			...
			<div
				...
				<div class="d-flex align-items-center gap-2">
					<select class="form-select form-select-sm"
						name="currentStageSelect"
						style="width: auto; border-radius: 8px; height: 38px;">
						<option value="">프로젝트 상태</option>
						<option value="진행중"
							<c:if test="${param.currentStage == '진행중'}">selected</c:if>>
							진행중</option>
						<option value="완료"
							<c:if test="${param.currentStage == '완료'}">selected</c:if>>
							완료</option>
						<option value="중단"
							<c:if test="${param.currentStage == '중단'}">selected</c:if>>
							중단</option>
					</select>
				</div>
				...
				<form action="/bts/project/list" method="get"
					...
					<input type="hidden" name="currentStage" id="currentStageHidden"
						value="${param.currentStage}" />
				...
				<table class="table align-middle text-center mb-0"
					...
					<tbody>
						...
						<c:if test="${empty projects}">
							<tr>
								<td colspan="6" style="padding: 40px 0; color: #aaa;">등록된
									프로젝트가 없습니다.</td>
							</tr>
						</c:if>
					</tbody>
			...
			<nav aria-label="Page navigation" class="mt-4">
				<ul class="pagination justify-content-center mb-0">
					...
					<li class="page-item ${page <= 1 ? 'disabled' : ''}"><a
						class="page-link"
						href="?page=${page-1}&projectName=${param.projectName}&currentStage=${param.currentStage}"
					...
					<c:forEach var="i" begin="1" end="${totalPage}">
						<li class="page-item ${i == page ? 'active' : ''}"><a
							class="page-link"
							href="?page=${i}&projectName=${param.projectName}&currentStage=${param.currentStage}"
				...
				.ready(
						function() {
							...
							const statusSelect = document
									.querySelector('select[name="currentStageSelect"]');
							const statusHidden = document
									.getElementById('currentStageHidden');
							const searchForm = document
									.querySelector('form[action="/bts/project/list"]');

							if (statusSelect && statusHidden && searchForm) {
